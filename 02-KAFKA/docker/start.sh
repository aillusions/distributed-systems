#!/bin/bash
# Bring up the 5-broker KRaft cluster + monitoring and wait until all five
...
echo "waiting for all 5 brokers ..."
until [ "$(docker compose exec -T broker1 \
  /opt/kafka/bin/kafka-broker-api-versions.sh \
  --bootstrap-server broker1:9092 2>/dev/null | grep -c '(id:')" = 5 ]; do
  sleep 1
done
echo "cluster up: 5 brokers ready."
...
echo "Brokers:     localhost:19092 .. localhost:19096"
